...
const Header = () => {
  ...
  return (
    <nav className='w-full flex bg-black justify-between items-center gap-1 lg:px-16 px-6 py-3 sticky top-0 z-50'>
      ...
      <button className='
        bg-orange-600 text-white px-10 py-3 rounded-full font-bold
          ...
          <ul className='flex flex-col justify-center items-center gap-2 w-full'>
            {navItems.map(({link, path}) => (
              <Link 
                key={path} 
                className='
                  text-white upercase font-semibold cursor-pointer p-3 rounded-lg
                  hover:bg-orange-600 hover:text-white w-full text-center
                ' 
                to={path} 
                spy={true} 
                offset={-100} 
